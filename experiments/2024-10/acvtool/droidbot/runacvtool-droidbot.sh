...
clean_up() {
    ...
    echo "cleaned up - droidbot processes might be dangling!"
    exit ${EXITNUM}
}
...
#
#
DBWD=${WORKINGDIR}/${APP}/droidbot
mkdir -p ${DBWD}
PATH="${PYENV_ROOT}/versions/aot-profiles/bin/:${PATH}" timeout $(($TIME+60))s droidbot -d ${DEVICEID} -a ${WORKINGDIR}/${APP}/instr_${APP}.apk -o ${DBWD} -t ${TIME} -keep_app || true # timeout can catch this and it returns nonzero
