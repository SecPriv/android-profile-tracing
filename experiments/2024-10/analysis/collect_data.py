...
def cachename_for_profcov(tool: str):
    return Path("cache/") / "profcov" / f"{tool}.pickle"

def cachename_for_acvtool(tool: str):
    return Path("cache/") / "acvtool" / f"{tool}.pickle"
...
def read_all_results_profcov(tool):
    resglob = f"../profcov/{tool}/results_{tool}-*/**/result.pickle"
    all_tsses = []
...
def read_all_results_acvtool(tool):
    ...
    coverages = []
    tools = []
    for resfile in tqdm.tqdm(glob.glob(f"../acvtool/{tool}/wd_{tool}-*/**/report/main_index.html")):
