...
mkdir -p wd/${APP}/droidbot
#PATH="$PYENV_ROOT/versions/aot-profiles/bin/:$PATH" droidbot
PATH="${PYENV_ROOT}/versions/aot-profiles/bin:${PATH}" timeout ${TIME}s droidbot -d ${DEVICE_ID} -a wd/${APP}/instr_${APP}.apk -o wd/${APP}/droidbot -t ${TIME} || true
